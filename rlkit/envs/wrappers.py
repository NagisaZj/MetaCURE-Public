...
class NormalizedBoxEnv(ProxyEnv, Serializable):
    ...
    def step(self, action):
        ...
        if not ((scaled_action<=ub).all() and (scaled_action>=lb).all()):
            scaled_action = np.zeros_like(scaled_action)
            print('!')
        assert (scaled_action<=ub).all() and (scaled_action>=lb).all()
...
class NormalizedBoxEnvTC(ProxyEnv, Serializable):
    """
    Normalize action to in [-1, 1].
    ...
    def reset(self, remaining_time, **kwargs):
        obs = self._wrapped_env.reset(**kwargs)
        self.remaining_time = remaining_time
        state = np.concatenate([obs,[self.remaining_time]])
        return state
