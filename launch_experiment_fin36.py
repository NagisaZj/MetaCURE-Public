...
from rlkit.torch.sac.sac import PEARLSoftActorCritic, ExpSACIter, ExpSACFin, ExpSACFin2, ExpSACFinSubtract, ExpSACFinSubtract2, ExpSACFinSubtract3
from rlkit.torch.sac.agent import PEARLAgent, ExpAgentFin, ExpAgentFin2, PEARLAgent2
...
def experiment(variant):
    ...
    transition_decoder = FlattenMlp(hidden_sizes=[net_size, net_size, net_size],
                             input_size=latent_dim*2 + obs_dim + action_dim,
                             output_size=obs_dim, )
    reward_predictor = SnailEncoder(
        hidden_sizes=[64, 64, 64],
        input_size=obs_dim + action_dim + 1,
        output_size=1,
        input_length=input_length
    )
    transition_predictor = SnailEncoder(
        hidden_sizes=[64, 64, 64],
        input_size=obs_dim + action_dim + 1,
        output_size=obs_dim,
        input_length=input_length
    ...
    '''reward_predictor = RNN(
        hidden_sizes=[64, 64, 64],
        input_size=obs_dim + action_dim + 1,
        output_size=1
    )
    transition_predictor = RNN(
        hidden_sizes=[64, 64, 64],
        input_size=obs_dim + action_dim + 1,
        output_size=obs_dim
    )'''
    ...
    baseline_reward_predictor = FlattenMlp(hidden_sizes=[net_size, net_size, net_size],
                             input_size= obs_dim + action_dim+1,
                             output_size=1, )
    baseline_trans_predictor = FlattenMlp(hidden_sizes=[net_size, net_size, net_size],
                                           input_size=obs_dim + action_dim + 1,
                                           output_size=obs_dim, )
    ...
    algorithm = ExpSACFinSubtract3(
        ...
        baseline_reward_predictor=baseline_reward_predictor,
        baseline_trans_predictor=baseline_trans_predictor,
        **variant['algo_params']
    )
